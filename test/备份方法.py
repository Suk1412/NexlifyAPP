...
class MouseWeaving():
    ...
    def create_grid(self):
        self.grid_list = [(i, j) for i in range(1, self.height + 1) for j in range(1,self.width + 1)]
        # for i in range(0, len(self.grid_list), self.width):
        #     print(*self.grid_list[i:i+self.width], end=" \n")
        ...
        for j in range(ex_length + a_standby_num + 1, ex_length + a_standby_num + a_correct_num + 1):
            try:
                if self.beam == "horizontal":
                    index = self.grid_list.index((vertical_value,j))
                if self.beam == "vertical":
                    index = self.grid_list.index((j,vertical_value))
    ...
    def signgle2(self):
        # value = 1
        # for i in range(0, len(grid_list), width):
        #     A = horizontal_dict[value]
        #     B = grid_list[i:i+width]
        #     value += 1
        #     da_cha(A,B)
        ...
    ...
    def convert(self):
        for _ in range(len(self.grid_list)):
            if self.grid_list[_] == -1:
                self.grid_list[_] = self.incorrect
            elif self.grid_list[_] == 1:
                 self.grid_list[_] = self.correct
            elif self.grid_list[_] != -1 or self.grid_list[_] != 1:
                self.grid_list[_] = self.standby
        ...
        height_length = 1
        for i in range(0, len(self.grid_list), self.width):
            print(f"{height_length:<2}", *self.grid_list[i:i+self.width], end=" \n")
            height_length += 1
