...
class Chipset(object):
    ...
    def chipset_error(self, cause):
        if cause is None:
            ...
            errno = cause[0]
        print(f"cause:{cause},errno:{errno}")
    ...
    def write_frame(self, frame):
        ...
        print("host ---> pn532:",frame.hex())
        self.transport.write(frame)
        # if frame != bytearray.fromhex("0000FF00FF00"):
        #     print("write data: ", decode(frame[5:-2]))
        # print("write data: ", decode(frame))
    ...
    def read_frame(self, timeout):
        ...
        # if frame != bytearray.fromhex("0000FF00FF00"):
        #     print("read  data: ", decode(frame[5:-2]))
        # print("read  data: ", decode(frame))
        return frame
    ...
    def in_communicate_thru(self, data, timeout):
        print("Data+","A"*30,data)
        data = self.command(0x42, data, timeout)
        print("Data+","B"*30,data)
        if timeout > 0:
...
class Device(device.Device):
    ...
    def send_cmd_recv_rsp(self, target, data, timeout, ttype = None):
        def bitrate(brty):
        ...
        print("write_register......send")
        self.chipset.write_register(*reg)
        print("write_register......close")
        ...
        try:
            if target.sens_res and not target.atr_res:
                if target.rid_res or ttype == "tt1":  # TT1
                    return self._tt1_send_cmd_recv_rsp(data, timeout+0.1)
                
                if target.sel_res[0] & 0x20 == 0x20 or ttype == "ttt":  # TT2_CMD
                    print("8"*30)
                    print("in_communicate_thru......send")
                    print("Data+","A"*30,data)
                    data = self.chipset.command(0x40, data, timeout)
                    print("Data+","B"*30,data)
                    if timeout > 0:
                        if data and data[0] == 0:
                            return data[1:]
                        else:
                            self.chipset.chipset_error(data)
                    print("in_communicate_thru......close")        
                ...
                if (target.sel_res[0] & 0x60 == 0x00) or ttype == "tt2":  # TT2
    ...
    def listen_tta(self, target, timeout):
        ...
        while time.time() < time_to_return:
            ...
            if brty != target.brty or len(data) < 2:
                self.log.debug("received bitrate does not match %s", target.brty)
                continue
